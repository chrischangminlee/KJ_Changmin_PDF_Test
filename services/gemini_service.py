...
def consolidate_items_with_llm(items, category: str, status_placeholder=None):
    """여러 페이지에서 수집된 항목들을 LLM으로 정규화/중복제거/정렬하여 1줄 항목들로 반환"""
    if not items:
        return []

    # 카테고리별 권장 출력 형식(추출 단계와 동일 형식 유지)
    formats = {
        "등급": "등급명",
        "기술자격": "자격명(등급) - 발급기관 - 취득(발급)일자",
        "학력": "학교명 - 학과/전공 - 졸업연도(가능하면)",
        "근무처": "근무처 - 부서/직무 - 직급/직위(가능하면) - 근무기간(가능하면)",
        "상훈": "상훈명 - 수여기관 - 수여일자(가능하면)",
        "벌점 및 제재사항": "구분 - 사유 - 일자(가능하면)",
        "교육훈련": "과정명 - 기관 - 기간 또는 시간(가능하면)",
    }

    format_hint = formats.get(category, "간결한 한 줄 요약")

    # 입력 목록을 텍스트로 구성
    lines_text = "\n".join([str(x) for x in items if str(x).strip()])

    prompt = f"""
당신은 문서에서 추출된 항목들을 정리하는 비서입니다.
아래 항목 목록을 중복 제거·병합·정규화하여 '{category}' 카테고리에 맞는 1줄 항목들로 정리하세요.

요청 카테고리: {category}
권장 형식: {format_hint}

정리 규칙:
- 의미상 동일하거나 중복된 항목은 하나로 통합(가능하면 가장 정보가 많은 형태 유지)
- 불완전한 항목은 보완 가능한 범위에서 정리하되, 새 정보는 만들지 마세요
- 일자는 YYYY-MM-DD 또는 YYYY.MM.DD처럼 일관되게 포맷(가능 시)
- 결과 순서는 문서상 자연스러운 순서(예: 최신순 또는 상위 먼저)로 정렬
